...
class F1Evaluator:
    ...
    def compute_names_f1(self, features, test_code_graph, real_targets, token_dictionary, result_file_all, result_file_badcase):
        """
        ...
        for i in xrange(features.shape[0]):
            if max([len(feai) for feai in features[i]]) > 160 or len(features[i]) > 160: 
                result = []
            else:
                result = self.model.predict_name(np.atleast_2d(features[i]), test_code_graph[i])

            confidences = [suggestion[1] for suggestion in result]
            if result:
                result_file_all.write( real_targets[i]+ "\t")
                result_file_all.write(','.join(result[0][0]) +  "\t"\
                            + str(token_precision_recall(result[0][0], real_targets[i].split(',')))[1:-1])
                result_file_all.write("\n")
            else:
                result_file_badcase.write(real_targets[i]+ "\t")
                result_file_badcase.write("0" +  "\t0, 0, 0")
                result_file_badcase.write("\n")
                
                result_file_all.write(real_targets[i]+ "\t")
                result_file_all.write("0" +  "\t0, 0, 0")
                result_file_all.write("\n")
