...
import re
sys.path.append('../')
from stmt2vec_attention.stmt2vec_model import CopyStmtRecurrentAttentionalModel
from stmt2vec_attention.f1_evaluator import F1Evaluator
from stmt2vec_attention.PDG_token_naming_data import TokenCodeNamingData
...
class ConvolutionalCopyAttentionalRecurrentLearner:
    ...
    def train(self, input_file, patience=5, max_epochs=1000, minibatch_size=50):
        ...
        train_name_targets, train_code_sentences, train_code_graph, train_code, train_target_is_unk, train_copy_vectors = train_data
        val_name_targets, val_code_sentences, val_code_graph, val_code, val_target_is_unk, val_copy_vectors = validation_data

        model = CopyStmtRecurrentAttentionalModel(self.hyperparameters, len(self.naming_data.all_tokens_dictionary),
                                   self.naming_data.name_empirical_dist, self.naming_data)
        
        def compute_validation_score_names():
            return model.log_prob_with_targets(val_code_sentences,val_code_graph, val_copy_vectors, val_target_is_unk, val_name_targets)
        ...
        for i in xrange(max_epochs):
            ...
            num_minibatches = int(ceil(float(len(train_name_targets)) / minibatch_size))-1  
            for j in xrange(num_minibatches):
                ...
                batch_code_graph = train_code_graph[name_batch_ids]
                for k in xrange(len(name_batch_ids)):
                    pos = name_batch_ids[k]
                    batch_code_sentencesk = np.array([np.array(code_sen, dtype=np.int32) for code_sen in batch_code_sentences[k]])
                    max_len = max([item.shape[0] for item in batch_code_sentences[k]])
                    ...
                    model.grad_accumulate(batch_code_sentencesk, graphk, train_copy_vectors[pos],
    ...
    def get_suggestions_for_next_subtoken(self, current_code, current_code_sentence, current_code_graph, predicted_target_tokens_so_far):
        copy_weights, copy_prob, name_logprobs = self.model.copy_probs(predicted_target_tokens_so_far, current_code_sentence, current_code_graph)
    ...
    def predict_name(self, code, code_graph, max_predicted_identifier_size=7, max_steps=100):
        ...
        code_sentence =  np.array([np.array(code_sen, dtype=np.int32) for code_sen in np.array(code_sens, dtype=np.object)])
        max_len = max([item.shape[0] for item in code_sentence])
        code_sentence = np.array([np.array([item for item in code_sen] + [self.naming_data.all_tokens_dictionary.get_id_or_unk(self.NONE) \
                                           for ii in range(max_len - code_sen.shape[0])], dtype=np.int32) for code_sen in code_sens])
        max_len = max([len(item) for item in code])
        code = [[item for item in code_sen] + [self.NONE for ii in range(max_len - len(code_sen))] for code_sen in code]
        code_extend = []
        for item in code: code_extend.extend(item)
        code = code_extend
        if code_sentence.shape[1] != 1:  code_sentence = code_sentence.T
        if code_sentence.shape[0] < code_sentence.shape[1]: code_sentence = code_sentence.T
        if code_sentence.shape[1] != code_graph.shape[1]: code_graph = code_graph[:,:code_sentence.shape[1],:code_sentence.shape[1]]
        ...
        while True:
            ...
            while len(possible_suggestions_stack) > 0:
                ...
                copy_prob, next_subtoken_suggestions, subtoken_target_logprob \
                    = self.get_suggestions_for_next_subtoken(code, code_sentence, code_graph, previous_subtokens)
    ...
    def load(filename):
        ...
        learner.model = CopyStmtRecurrentAttentionalModel(learner.hyperparameters, len(learner.naming_data.all_tokens_dictionary),
                             learner.naming_data.name_empirical_dist, learner.naming_data)
...
if __name__ == "__main__":
    ...
    params["train_file"] = "../json/" + file_name + "/train/train.json"
    params["test_file"] = "../json/" + file_name + "/test/test.json"
    
    input_file = params["train_file"]
    ...
    if True:
        ...
        result_file_badcases = open(params["test_file"][:-5] + "bad.txt", "w")
        ...
        test_name_targets, test_code_sentences,test_code_graph, test_code, test_target_is_unk, test_copy_vectors = test_data

        print "starting to test"
        
        eval = F1Evaluator(model2)
        point_suggestion_eval = eval.compute_names_f1(test_code, test_code_graph, original_names,
                                                      model2.naming_data.all_tokens_dictionary.get_all_names(), result_file_allcases, result_file_badcases)

        results = point_suggestion_eval.get_f1_at_all_ranks()
        print "[F1rank1, F1rank5]: ", results
        result_file_allcases.close()
        result_file_badcases.close()
