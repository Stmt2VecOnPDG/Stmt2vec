...
def gru_layer(gruW1,gruW2, gruU1, gruU2, grub1, grub2, gruWx, gruUx, grubx, state_below, init_state, prefix='gru', mask=None, mark_reverse = False, **kwargs):
    ...
    if init_state == None:
        init_state = T.alloc(np.float32(0.), dim)
    #init_state = T.cast(init_state, theano.config.floatX)
    if mask == None:
        mask = T.alloc(np.float32(1.0),  state_below.shape[0])
    ...
    def _step_slice(m_, x_1, x_2, xx_, h_, U1, U2, Ux):
        ...
        h = u * h_ + (np.float32(1.0) - u) * h
        #h = m_[:,None] * h + (np.float32(1.0)- m_)[:,None] * h_
    ...
    rval, updates = theano.scan(_step,
                                ...
                                non_sequences = [gruU1, gruU2, gruUx],
                                name='encoder_layers',
                                n_steps=nsteps,
                                profile=False,
                                strict=True)
